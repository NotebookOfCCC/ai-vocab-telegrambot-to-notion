...
New entries from bot go to Vocabulary_005.md onwards.
...
DATABASE_IDS = [
    ...
    "30e67845254b80e59ac4ec822cee3198",  # → Vocabulary_003.md
...
async def migrate_database(obsidian: ObsidianVocabHandler, entries: list[dict], part: int):
    """Write all entries to a specific Vocabulary_NNN.md file."""
    filename = f"Vocabulary_{part:03d}.md"

    header = f"""# Vocabulary Database - Part {part}
...
"""
    ...
    for i, entry in enumerate(entries, 1):
        row = obsidian._entry_to_row(entry, i)
        rows.append(row)

    content = header + "\n".join(rows) + "\n"

    filepath = f"{obsidian.BASE_PATH}/{filename}"
    await obsidian._put_file(filepath, content, f"migrate: {len(entries)} entries → {filename}")
    logger.info(f"✓ {filename}: {len(entries)} entries written")
...
async def main():
    ...
    for i, db_id in enumerate(DATABASE_IDS):
        ...
        try:
            ...
            if entries:
                await migrate_database(obsidian, entries, part)
            else:
                logger.warning(f"  No entries found in database {db_id[:8]}")
